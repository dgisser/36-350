...
```{r}
readLines("https://raw.githubusercontent.com/dgisser/36-350/master/README.md")
```
...
```{r}
library(devtools)
source_url("https://raw.githubusercontent.com/dgisser/36-350/master/hello_word.R")
```
...
f = function(x) {
  if(typeof(x) == "double") {
    warning("that's not the right type")
  }
  tolower(x) 
}
f(2)
...
```{r,error=TRUE}
f = function(x) {
  message("returns lower case string")
  if(typeof(x) == "double") {
    stop("that's not the right type")
  }
  tolower(x) 
...
f(2)
suppressMessages(f("HELLO, WORLD"))
```
...
```{r, error=TRUE}
library(testthat)
test_that(desc = "two empty strings", expect_silent(word.similarity("","")))
test_that(desc = "invalid input", expect_error(word.similarity(1,2)))
test_that(desc = "output type check", expect_type(word.similarity("hi","hi"),"double"))
test_that(desc = "silent error check", expect_silent(word.similarity("hi","hi")))
test_that(desc = "null check", expect_null(word.similarity("","")))
test_that(desc = "result check", expect_equal(word.similarity("aaa","zzz"),0))
```
